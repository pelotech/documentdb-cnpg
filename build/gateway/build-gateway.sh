...
: "${RUST_BUILDER:?set RUST_BUILDER in build/gateway/pins.env}"
: "${HARDENED_BASE_18:?set HARDENED_BASE_18 in build/gateway/pins.env}"
...
mapfile -t labels < <(ARTIFACT=gateway ./build/common/labels.sh)
...
docker build --platform "$plat" -f build/gateway/Dockerfile.gateway \
