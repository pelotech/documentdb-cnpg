#!/usr/bin/env bash
# Single source of truth for image references. Two images: `postgresql` (the engine
# images, one tag per Postgres major) and `extension` (the ImageVolume). Engine tags
# lead with the base's Postgres version because CNPG parses the imageName tag for the
# major version; the extension tag is not consumed as an imageName.
#
#   ARTIFACT=engine   PG_MAJOR=17  REF=<ref>  -> ${IMAGE_REPO}/postgresql:<pgver>-<ref>
...
case "$ARTIFACT" in
  engine)
    ...
    echo "${IMAGE_REPO}/postgresql:${pgver}-${REF}" ;;
  imagevol)
